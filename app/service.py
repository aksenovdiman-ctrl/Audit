    ...
    async def _deliver_ready(
        ...
        *,
        ...
        try:
            ...
            await self._notify_admins(
                (
                    "Результат передан в SalesBot callback.\n"
                    f"client_id: {client_id}\n"
                    f"image_attached: {'yes' if image_url else 'no'}"
                )
            )
        except ExternalAPIError as exc:
            await self._notify_admins(
...
Они являются ГЛАВНЫМ источником содержания.
Используй из них:
— аватар в стилизованном, упрощенном виде
— ник и имя в адаптированном handwritten-виде
— смысл био и позиционирования
— фактические блоки статистики, закрепов и хайлайтсов
— визуальное настроение ленты
— типы контента
— характер оформления профиля
— композицию и темы постов в стилизованном виде

Очень важно:
— итоговая страница должна читаться как аудит именно ЭТОГО профиля по скринам 1 и 2
— не подменяй содержание скринов выдуманным профилем
— не делай универсальный шаблон с абстрактным брендом вместо реального профиля
— если на скринах видны конкретные темы, рубрики, визуальные акценты или элементы упаковки, отрази их в стилизованном виде на итоговой странице
— можно перерисовать интерфейс и фото упрощенно, но содержание должно быть основано именно на скринах 1 и 2
...
Не переноси из 3 изображения:
— текст
— конкретные блоки
— заголовки
— порядок пунктов
— содержание заметок
— аватар, ник, посты, темы или подписи

Сделай итог как полноценную handwritten-страницу маркетингового разбора реального профиля со скринов 1 и 2.
Не воспроизводи буквально реальные лица или точный интерфейс Instagram пиксель-в-пиксель.
Но:
— можно стилизованно перерисовать аватар
— можно адаптированно показать ник, имя и био
— можно показать узнаваемые для этого профиля блоки контента, хайлайтсов и ленты
— можно передать реальные темы и позиционирование профиля

Не подставляй в карточку вымышленный бренд {brand_name}, если на скринах есть конкретный профиль.
Используй {brand_name} только если на скринах не удается прочитать название вообще.
...
— стилизованный блок профиля
— читаемый ник/имя в handwritten-подаче, если они различимы
— статистику, близкую к увиденной на скринах
— краткое описание позиционирования по био
— иконки хайлайтсов по мотивам реального профиля
— мини-превью постов по мотивам реальной ленты
— блоки и инсайты, связанные именно с темами, которые видны на скринах
...
Главный приоритет:
1. Содержание и особенности профиля со скринов 1 и 2
2. Только потом стиль и материалы из 3 изображения
""".strip()
